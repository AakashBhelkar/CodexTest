...
### `tsx` error: `src/app.ts ... The symbol "app" has already been declared`

This usually means `src/app.ts` contains duplicated blocks after a bad merge (two `const app = express()` declarations).

Quick fix:
1. Open `src/app.ts`
2. Keep only **one** app bootstrap block
3. Ensure file ends with a single `export { app };`
